...
function displayProfileInfo(data) {
  ...
  document.getElementById("age-info").innerText = `${computeAge(
    data.birthday
  )}`;
  ...
  try {
    ...
    if (response.ok) {
      if (result.successMessage) {
        successMessageDiv.innerText = result.successMessage;
      ...
      if (profileResponse.ok) {
        ...
        console.error(
          "Error fetching updated patient profile:",
          profileResponse.status,
          profileResponse.statusText
        );
        errorMessageDiv.innerText = "Failed to refresh profile data.";
    ...
    errorMessageDiv.innerText =
      "Failed to update profile. Please check your network connection or try again later.";
    errorMessageDiv.style.display = "block";
  }
}
  ...
  if (!isConfirmed) {
    ...
    if (!pictureInput.files || pictureInput.files.length === 0) {
      ...
      if (!response) {
        ...
        document.getElementById("sidebar-profile-picture").src =
          uploadedImageUrl;
...
async function deleteProfilePicture() {
  try {
    ...
    if (response.ok) {
      ...
      console.error(
        "Failed to delete profile picture on the server:",
        response.statusText
      );
