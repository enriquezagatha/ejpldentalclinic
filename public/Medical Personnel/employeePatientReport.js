document.addEventListener("DOMContentLoaded", function () {
    ...
    async function fetchReport() {
        try {
            ...
            const response = await fetch(`/api/appointments/appointment-report?startDate=${startDate}&endDate=${endDate}`);
            ...
            appointments.forEach((appointment) => {
                const row = `
                    <tr>
                        ...
                        <td>${new Date(appointment.preferredDate).toLocaleDateString()}</td> <!-- ✅ Format Date -->
                    </tr>
                `;
            ...
            patientList.innerHTML = '<tr><td colspan="3">Error loading data.</td></tr>';
            totalPatientsElement.textContent = "0";
        }
    }    
