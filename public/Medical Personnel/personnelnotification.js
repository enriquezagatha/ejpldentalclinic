...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (e) => {
    ...
    const isClickOnActionButtonOrCheckbox = e.target.closest(
      "#mark-read-btn, #mark-unread-btn, #delete-btn, .notification-checkbox"
    );
    ...
    if (
      ...
      !isClickOnBell &&
      !isClickOnActionButtonOrCheckbox
    ) {
...
function displayNotifications(notifications) {
  ...
  if (notifications.length === 0) {
    const noNotificationsHTML =
      '<li style="padding: 10px;">No notifications yet.</li>';
  ...
  notifications.forEach((notification) => {
    const itemHTML = `
            <li class="notification-item flex items-start gap-4 p-4 border-b border-gray-200">
                <input type="checkbox" class="notification-checkbox mt-1" data-id="${
                  notification._id
                }">
                <div class="flex-1">
                    <strong class="block text-lg font-semibold text-gray-800">${
                      notification.title
                    }</strong>
